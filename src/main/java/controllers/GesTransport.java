...
public class GesTransport implements Initializable {
    ...
    void addTransport(ActionEvent event) {
        try {
            ...
            if (transportCapacityField.getText().isEmpty()) {
                showErrorAlert("Veuillez entrer une capacité.");
                return;
            }
    ...
    private void handleSubmitUpdate(Transport transport) {
        try {
            ...
            // Validate capacity and price as numbers
            int capacity = 0;
            double price = 0;
            try {
                capacity = Integer.parseInt(transportCapacityField.getText());
                price = Double.parseDouble(transportPriceField.getText());
            } catch (NumberFormatException e) {
                showErrorAlert("Veuillez entrer des valeurs valides pour la capacité et le prix.");
                return;
            }
            ...
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setTitle("Succès");
            alert.setContentText("Transport modifié avec succès !");
            alert.showAndWait();

        } catch (Exception e) {
            showErrorAlert("Erreur lors de la mise à jour du transport. " + e.getMessage());
        }
    }
